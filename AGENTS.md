...
Run `npm test` and `npm run build` after behavioral changes. Test model mismatch, invalid input, atomic multi-part changes and undo/redo. Keep model changes separately traceable from code changes. Preserve attribution and the model's non-commercial ShareAlike license. No backend, telemetry or printer integration is implied by this project.
