...
1. Export a GLB with one mesh node per independently editable part instance. Names must be stable ASCII IDs, unique and equal to manifest IDs. Merge multiple primitives of a physical part before export. Bake transforms, use millimeters, Y up and front +Z. Negative/scaled parent transforms should be baked.
