...
An optional `mobileGeometryUrl` points to a lower-cost display GLB with the same stable mesh IDs. Microduck's mobile mesh is generated by `scripts/build_mobile_model.py` from this project's full display GLB, using trimesh and fast-simplification. Normals are baked offline. Both assets are previews and never manufacturing sources. Regenerate the mobile asset after rebuilding the full one.

The shipped GLBs carry `EXT_meshopt_compression`, which takes the display geometry to roughly a third of its original transfer size. `scripts/build_meshopt.ts` applies it:
