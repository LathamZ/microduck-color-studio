...
- **Fit the other feet.** The top bar switches the duck between feet and clip-on skates. Whichever set is not fitted is laid out on the floor beside the duck, about a body's width away, so both sets can be selected and coloured at once instead of only what the duck happens to be wearing. Once the skates are on, the actions that need a foot flat on the floor leave the menu.
- **See it move.** The action button plays a looping walk with head shakes and beak opening; hover it to pick a single motion: walk, sit and stand, kick, scoop, get back up, head shake, head tilt or beak. The walk is a waddle: a duck's legs are short and set wide, so its weight cannot pass between its feet — the trunk leans and travels out over whichever foot is carrying it, and the hips open and close against that so the planted foot stays where it was put, which is the knee rotation a waddling bird uses. The head counter-rolls to hold its eyes level. Each step is solved with inverse kinematics — the foot path comes first, then the hip, knee and ankle servos follow it, so the planted foot never leaves the ground. Sitting lowers the hips onto feet that do not move; the scoop is done by the neck and jaw alone; getting back up turns the body about whatever is actually touching the floor — its heels first, then its rump — so the contact moves as it goes over rather than the duck swinging on one imaginary hinge. The motion stops when parts are exploded.
