...
- **See it move.** The action button plays a looping walk with head shakes and beak opening; hover it to pick a single motion: walk, sit and stand, kick, scoop, get back up, head shake, head tilt or beak. The walk is a waddle: a duck's legs are short and set wide, so its weight cannot pass between its feet — the trunk leans and travels out over whichever foot is carrying it, and the hips open and close against that so the planted foot stays where it was put, which is the knee rotation a waddling bird uses. The head counter-rolls to hold its eyes level. Each step is solved with inverse kinematics — the foot path comes first, then the hip, knee and ankle servos follow it, so the planted foot never leaves the ground. Sitting lowers the hips onto feet that do not move; the scoop is done by the neck and jaw alone; getting back up rolls the whole body about the point it touches the floor. The motion stops when parts are exploded.
