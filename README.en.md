...
- **Fit the other feet.** The top bar switches the duck between feet and clip-on skates. Whichever set is not fitted is laid out on the floor beside the duck, about a body's width away, so both sets can be selected and coloured at once instead of only what the duck happens to be wearing. Exploding the assembly opens both sets at once, each about its own middle — the spare set is lying on the floor, and spreading it about the duck's middle would drag it under. While an action plays, that set fades out and the camera draws in to the duck itself, then it is laid back down when the action stops. The switch itself is out of reach while an action plays and while the parts are exploded: in the first case a pose is holding the parts, in the second the set that would have to be laid out on the floor is already off the rig, and a swap in either one would put the parts down in the wrong place. Once the skates are on, the actions that need a foot flat on the floor leave the menu.
- **See it move.** The action button plays a looping walk with head shakes and beak opening; hover it to pick a single motion: walk, sit and stand, kick, scoop, get back up, head shake, head tilt or beak. The walk is a waddle: a duck's legs are short and set wide, so its weight cannot pass between its feet — the trunk leans and travels out over whichever foot is carrying it, and the hips open and close against that so the planted foot stays where it was put, which is the knee rotation a waddling bird uses. The head counter-rolls to hold its eyes level. Each step is solved with inverse kinematics — the foot path comes first, then the hip, knee and ankle servos follow it, so the planted foot never leaves the ground. Sitting lowers the hips onto feet that do not move; the scoop is done by the neck and jaw alone; getting back up turns the body about whatever is actually touching the floor — its heels, then its rump, then the back of its head — so the contact moves as it goes over rather than the duck swinging on one imaginary hinge, and since its legs are in the air it cannot simply stand: it presses the back of its head to the floor and levers itself up on its own neck, the head staying put while the trunk rides up over it until the feet are under it again. The motion stops when parts are exploded.
...
This is an appearance configurator, not a slicer or an assembly-fit validator. The bundled geometry comes from the original **XL330 simulation assembly**, retaining about 796,800 source triangles. It is not an exact HD1910 assembly. The top bar switches between **feet and skates**: the roller module is the official simulator's own swap-in parts — the roller ankles, the blades and four wheels — placed mesh by mesh from the official MuJoCo model. The roller ankles stand 10 mm taller than the standard ones, so the duck rides higher on them.
