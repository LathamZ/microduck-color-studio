...
- **See it move.** The action button plays a looping walk with head shakes and beak opening; hover it to pick a single motion: walk, sit and stand, kick, scoop, get back up, head shake, head tilt or beak. Each step is solved with inverse kinematics — the foot path comes first, then the hip, knee and ankle servos follow it, so the planted foot never leaves the ground. Sitting lowers the hips onto feet that do not move; the scoop is done by the neck and jaw alone; getting back up rolls the whole body about the point it touches the floor. The motion stops when parts are exploded.
